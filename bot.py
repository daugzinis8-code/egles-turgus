...
TOKEN = os.getenv("8667731379:AAHGElYC3kut4M5zcXhPJy_OY35nrwJHeSA")
if not TOKEN:
    raise ValueError("TOKEN nerastas! Render'e pridėk Environment Variable TOKEN")
...
def fmt(x):
    return f"{x.get('pavadinimas','-')}\n{x.get('kaina','-')}\n{x.get('vieta','-')}\n@{x.get('username') or x.get('user','')}"

@bot.message_handler(commands=['start'])
def start(m):
    user_data.pop(m.chat.id,None)
    bot.send_message(m.chat.id,"Eglės turgus Klaipėda 24/7! Pasirink:",reply_markup=main_kb())
...
def s1(m):
    ...
    bot.send_message(m.chat.id,"Ką ieškai? Parašyk žodį:")

@bot.message_handler(func=lambda m: m.text=="🟢 PERKU")
def perku(m):
    db=load_db()
    if not db: bot.send_message(m.chat.id,"Tuščia!",reply_markup=main_kb()); return
...
def bal(m):
    db=load_db(); bot.send_message(m.chat.id,f"Skelbimų: {len([x for x in db if x.get('chat_id')==m.chat.id])} / Viso: {len(db)}")
...
def steps(m):
    ...
    if d['a']=='search':
        q=m.text.lower()[:4]; db=load_db()
        found=[x for x in db if q in x.get('pavadinimas','').lower()]
        if not found: bot.send_message(m.chat.id,f"Nieko pagal '{m.text}'",reply_markup=main_kb())
        else:
            for x in found[-10:]: bot.send_message(m.chat.id, fmt(x))
    ...
    if d['a']=='sell':
        ...
        if d['s']==3:
            db=load_db(); db.append({"id":int(time.time()*1000),"pavadinimas":d['pav'],"kaina":d['kaina'],"vieta":m.text,"user":m.from_user.first_name,"username":m.from_user.username,"chat_id":m.chat.id})
            save_db(db); bot.send_message(m.chat.id,"✅ Įdėta!",reply_markup=main_kb()); user_data.pop(m.chat.id,None)

@bot.callback_query_handler(func=lambda c: c.data.startswith("del_"))
def dell(c):
    nid=int(c.data.split("_")[1]); db=load_db(); db=[x for x in db if x.get('id')!=nid]; save_db(db); bot.answer_callback_query(c.id,"Ištrinta")
